...
else (if(data == "field") mapped map ($ mapObject (($$):$) if(!(value contains ($$ as String))) ) else (mapped map (if(valueSet($) contains value) "" else $)) - "")
fun reqFieldCheck(value) = if(value map ($.ProductId) contains null) (fail("Required field ProductId is not found")) else value
---
reqFieldCheck(if(function == null) (mapped) else dynamic(function,data,value))